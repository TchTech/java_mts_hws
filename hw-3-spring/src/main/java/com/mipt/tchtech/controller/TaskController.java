...
public class TaskController {
    ...
    @Operation(summary = "Получить все задачи с вложениями (без N+1)")
    public ResponseEntity<List<TaskResponseDto>> getAllTasksWithAttachments() {
        return ResponseEntity.ok(taskService.getAllTasksWithAttachments());
    }

    @GetMapping("/upcoming")
    @Operation(summary = "Получить задачи с дедлайном в ближайшие 7 дней")
    public ResponseEntity<List<TaskResponseDto>> getUpcoming() {
    ...
    @Operation(summary = "Пакетно завершить задачи")
    public ResponseEntity<Void> bulkComplete(@RequestBody List<Long> ids) {
